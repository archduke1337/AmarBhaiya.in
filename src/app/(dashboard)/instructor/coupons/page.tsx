...
export default async function InstructorCouponsPage() {
  ...
  const [coupons, instructorCourses] = await Promise.all([
    getCoupons(),
  ...
  return (
    <div className="flex flex-col gap-8 max-w-5xl">
      <PageHeader
        eyebrow="Instructor · Coupons"
        title="My Coupons"
        description={`${coupons.length} total · ${activeCoupons.length} active`}
      />
        ...
        <div className="flex items-center gap-2 border-b border-border/40 px-5 py-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Offer discounts on your own courses.
            </p>
          </div>
        </div>
      ...
      {coupons.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-border/40 bg-surface">
          ...
          <div className="hidden items-center gap-3 border-b border-border/40 px-5 py-3 text-xs uppercase tracking-[0.15em] text-muted-foreground md:grid md:grid-cols-[100px_1fr_80px_80px_100px_80px_80px]">
            <span>Code</span>
            <span>Course</span>
            <span>Type</span>
            <span>Value</span>
          ...
          <div className="divide-y divide-border/40">
            {coupons.map((coupon) => (
              <div
                key={coupon.id}
                className="flex flex-col gap-2 px-5 py-4 transition-colors hover:bg-accent/30 md:grid md:grid-cols-[100px_1fr_80px_80px_100px_80px_80px] md:items-center md:gap-3"
              >
                <div>
